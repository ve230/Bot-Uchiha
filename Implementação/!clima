module.exports = {
    ...
    execute: async (sock, msg, args) => {
        ...
        const resp = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${cidade}&appid=${apiKey}&lang=pt&units=metric`);
        const data = await resp.json();
        if (data.cod !== 200) return sock.sendMessage(jid, { text: "Cidade não encontrada." });
