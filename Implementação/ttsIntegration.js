...
module.exports = {
  ...
  async onMessage({ sock, msg, text }) {
    try {
      ...
      let payload = null;
      if (lower.toLowerCase().startsWith("voz:")) payload = lower.slice(4).trim();
      ...
      if (this.provider === "elevenlabs") {
        try {
          ...
          await sock.sendMessage(jid, { audio: data, mimetype: "audio/mpeg", ptt: false });
          fs.unlinkSync(tmpFile);
        } catch (e) {
          console.error("elevenlabs tts error", e);
          await sock.sendMessage(jid, { text: "❌ Falha no TTS: " + (e.message || e.toString()) });
        }
      } else if (this.provider === "openai") {
        // Placeholder: OpenAI TTS flow (user must implement based on chosen model)
        await sock.sendMessage(jid, { text: "TTS via OpenAI não configurado — configure config.tts.provider e chaves." });
      } else {
        // fallback: no TTS — reply with instruction
        await sock.sendMessage(jid, { text: "TTS não configurado. Configure config.tts.provider = 'elevenlabs' e chave em config.js ou env." });
      }
    } catch (e) {
