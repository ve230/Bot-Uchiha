...
module.exports = {
    ...
    run: async ({ sock, msg, args }) => {
        ...
        if (idx < 0 || idx >= shopItems.length) return sock.sendMessage(jid, { text: "⚠️ Item inválido!" });

        const item = shopItems[idx];
        const bal = getBalance(userId);

        if (bal.wallet < item.price) return sock.sendMessage(jid, { text: "⚠️ Saldo insuficiente!" });
        ...
        await sock.sendMessage(jid, { text: `✅ Você comprou: ${item.name} por ${item.price} coins!` });
    }
};