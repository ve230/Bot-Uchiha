      ...
      if (onlyDigits.length < 8) {
        ...
        if (this.autoBan) {
          try {
            ...
            await sock.sendMessage(msg.key.remoteJid, { text: `✅ Removido por antiFake: @${phone}`, mentions: [sender] });
          } catch (e) { /* ignore */ }
        }
      } else if (this.allowed.length > 0) {
        const matched = this.allowed.some(code => onlyDigits.startsWith(code));
        if (!matched) {
          await sock.sendMessage(msg.key.remoteJid, {
