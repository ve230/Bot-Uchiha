...
module.exports = {
  ...
  async onMessage({ sock, msg, text }) {
    try {
      ...
      if (!content) return await sock.sendMessage(jid, { text: "Escreva algo após `voz:` para eu responder com voz." });

      // If TTS not configured -> just send textual reply and hint
      if (!this.ttsEnabled) {
        await sock.sendMessage(jid, { text: `🔊 Resposta (texto): ${content}\n\nPara voz real configure OPENAI_TTS_KEY ou outro serviço TTS.` });
        return;
      }
      ...
      await sock.sendMessage(jid, { text: "✅ TTS configurado, mas ainda não implementado no bot. Configure e implemente a chamada ao serviço TTS." });
    } catch (e) {
