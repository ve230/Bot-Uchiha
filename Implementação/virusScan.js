  ...
  async onMessage({ sock, msg }) {
    try {
      ...
      if (this.blockedExt.includes(ext)) {
        await sock.sendMessage(jid, { text: `⚠️ Arquivo com extensão bloqueada detectado: .${ext}` });
      ...
      if (fileLen > this.maxFileBytes) {
        await sock.sendMessage(jid, { text: `⚠️ Arquivo muito grande (${Math.round(fileLen/1024/1024)}MB). Limite: ${Math.round(this.maxFileBytes/1024/1024)}MB` });
        try { await sock.sendMessage(jid, { delete: msg.key }); } catch(e){/*ignore*/}
      }
    } catch (e) {
