  ...
  bind({ sock }) {
    sock.ev.on("group-participants.update", async (update) => {
      try {
        ...
        const background = this.templatePath && fs.existsSync(this.templatePath) ? await Jimp.read(this.templatePath) : new Jimp(1024, 512, "#0a0a0a");
        ...
        await sock.sendMessage(jid, { image: data, caption: `🔥 @${user.split("@")[0]}, você entrou no clã Uchiha. Prepare-se para observar e respeitar.`, mentions: [user] });
